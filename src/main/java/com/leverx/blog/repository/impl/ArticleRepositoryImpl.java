...
public class ArticleRepositoryImpl implements ArticleRepository {
    ...
    public List<Article> findAllPublicArticles() {
        ...
        Predicate predicateStatus = criteriaBuilder.equal(
                articleRoot.get(Article_.STATUS), Status.PUBLIC.toString());
        criteriaQuery.select(articleRoot).where(predicateStatus);
    ...
    public List<Article> findByName(String name) {
        ...
        Predicate predicateTitle = criteriaBuilder.equal(
                articleRoot.get(Article_.TITLE), name);
        criteriaQuery.select(articleRoot).where(predicateTitle);
        return entityManager.createQuery(criteriaQuery).getResultList();
    ...
    public List<Article> findByStatus(String status) {
        ...
        Predicate predicateStatus = criteriaBuilder.equal(
                articleRoot.get(Article_.STATUS), status);
        criteriaQuery.select(articleRoot).where(predicateStatus);
        return entityManager.createQuery(criteriaQuery).getResultList();
